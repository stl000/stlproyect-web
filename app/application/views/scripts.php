...
const WebSocket_URL = 'wss://stlproyect.tk:8094/mqtt';
...
client.on('connect', () => {
  ...
  client.subscribe(device_topic + "data", { qos: 0 }, (error) => {
    if(error){
      ...
      console.log("Subscribe at "+device_topic+"ok");
    }
...
client.on('message', (topic, message) => {
  ...
  if (topic == device_topic+"data"){
    ...
    if(autoPanel == "1"){
      ...
      $("#display_autoPanel").prop('checked',"");
    }
...
function autoPanel_change(){
  ...
  {
    client.publish(device_topic + 'actions/aut',"1");
  }else{
    client.publish(device_topic + 'actions/aut',"0");
  }
}
